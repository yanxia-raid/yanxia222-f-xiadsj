...
- **云端助手**：同一套逻辑部署为用户自己 Supabase 项目里的 Edge Function，由 pg_cron 每 10 秒触发一次，不需要电脑常开。部署方法见 `docs/weixin-cloud-assistant.md`。
...
- 文本消息会复用小手机的状态清理和分段策略；本地版支持图片 / 语音等媒体回复，云端版目前把媒体协议降级为文字（Deno 环境的媒体上传路径验证后再开放）。
