...
export function TavernControlPanel({ char, onSaved }: { char: Character; onSaved?: (character: Character) => void }) {
  ...
  return <div className="mt-4 rounded-3xl border-2 border-[var(--c-text)]/15 bg-black/[.02] p-3 dark:bg-white/[.02]">
    ...
    <div className="mt-3 space-y-3">
      {tab === "overview" && <><Section title="运行能力"><div className="flex flex-wrap gap-1.5"><Badge>开场白 {greetings.filter(Boolean).length}</Badge><Badge>World Book {bookEntries.length}</Badge><Badge>Regex {regexEntries.length}</Badge>{binding?.statusBarId && <Badge>Status Bar</Badge>}<Badge>{profile?.markup || "plain"}</Badge>{profile?.customCss && <Badge>Custom CSS</Badge>}{profile?.hasStructuredOutput && <Badge>Structured Output</Badge>}</div></Section><Section title="卡片元数据"><div className="grid grid-cols-2 gap-2 ts-10"><div>creator：<b>{String(d.creator || "—")}</b></div><div>version：<b>{String(d.character_version || "—")}</b></div><div>source：<b>{card.sourceFormat}</b></div><div>spec：<b>{card.spec}</b></div><div className="col-span-2">tags：<b>{Array.isArray(d.tags) ? d.tags.join(" · ") || "—" : "—"}</b></div></div></Section></>}
