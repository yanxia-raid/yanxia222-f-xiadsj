...
export function VoiceSettings() {
    ...
    // 当编辑模态框打开时，检查已保存的音色 ID 是否在下拉选项中
    // 如果不在，自动切换到手动输入模式，以便用户能看到并复制已保存的音色 ID
    useEffect(() => {
        if (!editingId) return;
        const config = configs.find(c => c.id === editingId);
        if (!config) return;
        // 如果已经有 manualVoiceIds 标记，不需要再处理
        if (manualVoiceIds[config.id]) return;
        // FishAudio 始终使用手动输入模式
        if (config.provider === "FishAudio") {
        ...
        // 对于 Minimax/OpenAI，检查 defaultVoice 是否在下拉选项中
        const options = voiceOptionsForConfig(config, fetchedVoices);
        const isInOptions = options.some(v => v.id === config.defaultVoice);
        if (config.defaultVoice && !isInOptions) {
            setManualVoiceIds(prev => ({ ...prev, [config.id]: true }));
        }
