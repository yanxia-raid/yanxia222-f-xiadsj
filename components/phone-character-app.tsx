...
function CharListView({
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      // SillyTavern Character Card import is handled before the legacy parser
      // so greeting/scenario/example fields are never discarded.
      ...
      if (isTavernFile) {
        try {
          ...
          onNotice(`已导入酒馆角色卡「${result.card.data.name}」${warningText}。点击画布放置角色`);
          return;
        } catch (tavernError) {
          if (lowerName.endsWith(".charx")) throw tavernError;
          console.warn("Tavern parser failed; trying legacy character parser.", tavernError);
        }
      }
      ...
      if (file.type === "application/json" || lowerName.endsWith(".json")) {
        ...
        onNotice("请选择 .json、.png 或 .charx 文件");
      }
    } catch (e) {
      if (e instanceof Error && e.message === CHAR_BLOCKED_FIELDS) {
        setImportError("角色卡解析失败，请确认文件是有效的 PNG/JSON/CHARX 酒馆角色卡");
      } else {
        console.error("Character import failed", e);
        setImportError(e instanceof Error ? `导入失败：${e.message}` : "解析失败，请检查文件格式");
      }
    }
  }
        ...
        footer={
          <div className="char-bottom-bar flex justify-center pb-8">
            <div className="wt-bottom-pill">
              ...
              <input
                ref={fileRef} type="file" accept=".json,.png,.charx,image/png,application/json" className="hidden"
                onChange={async (e) => {
