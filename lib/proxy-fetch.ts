// 服务端出网请求的代理感知 fetch(通用版,tripo 路由的同款方案)。
// 本地 dev 时开发机常靠系统代理才能出网,而 Node 的 fetch 不读代理环境变量
// (curl 读,所以「curl 通、route 超时/500」)。这里检测到 HTTPS_PROXY/HTTP_PROXY
// 就挂 undici ProxyAgent;线上(Netlify)无代理环境变量,行为与原生 fetch 完全一致。
