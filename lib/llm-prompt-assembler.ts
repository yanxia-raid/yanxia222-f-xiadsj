...
function getMarkerContent(
    ...
    switch (identifier) {
        case "charDescription":
            // For an imported Tavern card, its own card fields are authoritative.
            // Do not prepend the app's legacy "You are ..." wrapper or duplicate the
            // card description; the Tavern-owned block is injected by the assembler.
            if ((character as Character & { tavernCard?: unknown }).tavernCard) return null;
...
export function assemblePromptPayload(input: AssemblerInput): LLMMessage[] {
    ...
    // Tavern runtime is character-scoped: imported world books/regex never become
    // global user settings and therefore cannot leak into another character.
    const tavernRuntime = mergeTavernRuntimeConfig(character, worldBooks, regexes);
    const activeWorldBooks = tavernRuntime.worldBooks;
    const activeRegexes = tavernRuntime.regexes;
    const tavernCard = tavernRuntime.card;
    ...
    if (preset) {
        ...
        if (tavernCard) {
            const tavernSystem = buildCharacterOwnedTavernSystem(tavernCard, resolvedUserName);
            if (tavernSystem) {
                const engineText = new MacroEngine(character.name, resolvedUserName);
                applyTimeContextToMacroEngine(engineText, promptTimeContext);
                const expanded = postProcessTrim(engineText.expand(tavernSystem)).trim();
                if (expanded) {
                    blocks.push({
                        text: expanded,
                        role: "system",
        ...
        // ST first_mes is an assistant turn that exists before the user's first
        // message. Inject it only for a genuinely new one-message turn, so it is
        // not repeated on later generations. The chat UI can separately choose an
        // alternate greeting; the prompt remains deterministic here.
        if (tavernCard && history.length === 1 && history[0]?.role === "user") {
            const greeting = getTavernGreeting(tavernCard);
            if (greeting) {
                const greetingEngine = new MacroEngine(character.name, resolvedUserName);
                applyTimeContextToMacroEngine(greetingEngine, promptTimeContext);
                const expandedGreeting = postProcessTrim(greetingEngine.expand(greeting)).trim();
                if (expandedGreeting) {
                    blocks.push({
                        text: expandedGreeting,
                        role: "assistant",
                        depth: history.length + 2,
                        order: -20,
                        marker: "tavern:first_mes",
                    });
                }
            }
        }
        ...
        for (let promptIndex = 0; promptIndex < processingOrder.length; promptIndex += 1) {
            ...
            if (p.marker) {
                ...
                let markerContent = getMarkerContent(
                    ...
                    coreMemories,
                    longTermMemories,
                    activeRegexes, { macroEngine: engine, activeTags },
        ...
        wbAtDepthEntries.forEach(entry => {
            ...
            content = postProcessTrim(content).trim();
            if (!content) return;
            content = applyWorldInfoRegex(content, activeRegexes, { macroEngine: engine, activeTags });
            blocks.push({
                text: content,
    ...
    // ST post-history instructions belong immediately after the history range.
    if (tavernCard) {
        const postHistory = buildCharacterOwnedTavernPostHistory(tavernCard, resolvedUserName);
        if (postHistory) {
            const postEngine = new MacroEngine(character.name, resolvedUserName);
            applyTimeContextToMacroEngine(postEngine, promptTimeContext);
            const expanded = postProcessTrim(postEngine.expand(postHistory)).trim();
            if (expanded) {
                blocks.push({
                    text: expanded,
                    role: "system",
                    depth: 0,
                    order: 2000,
                    marker: "tavern:post_history_instructions",
                });
            }
        }
    }
    ...
    blocks.forEach(b => {
        const inputCtx: RegexContext = b.fromHistory
            ...
            : { activeTags };
        const processedText = b.role === "tool" ? b.text : applyInputRegex(b.text, activeRegexes, inputCtx);
...
export function isWorldBookEntryActivated(entry: WorldBookEntry, contextText: string): boolean {
    ...
    const extra = entry as WorldBookEntry & {
        tavernSecondaryKeys?: string[];
        tavernSelective?: boolean;
        tavernCaseSensitive?: boolean;
    };
    const keys = entry.key.split(",").map(k => k.trim()).filter(Boolean);
    const secondary = Array.isArray(extra.tavernSecondaryKeys) ? extra.tavernSecondaryKeys.filter(Boolean) : [];
    if (keys.length === 0) return false;

    const caseSensitive = extra.tavernCaseSensitive === true;
    const matches = (key: string) => {
        if (entry.use_regex) {
            try {
                const flags = caseSensitive ? "" : "i";
                return new RegExp(key, flags).test(contextText);
            } catch {
                return false;
            }
        }
        return caseSensitive ? contextText.includes(key) : contextText.toLowerCase().includes(key.toLowerCase());
    };

    if (!keys.some(matches)) return false;
    if (secondary.length && extra.tavernSelective !== false && !secondary.some(matches)) return false;
