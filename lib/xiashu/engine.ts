...
import { loadApiConfigs, loadBindingConfig, loadPresets, loadRegexes, loadTavernStatusBars, resolveBinding } from "@/lib/settings-storage";
...
export function buildXiaShuPrompt(characterIds: string[], worldId: string, history: XiaShuMessage[]): LLMMessage[] {
  const characters = getXiaShuCharacters(characterIds);
  const groups = loadCharacterWorldGroups();
  const world = groups.find(g => g.id === worldId);
  const userName = "用户";
  const blocks: string[] = [
  ...
  for (const m of history.slice(-60)) {
    messages.push({ role: m.role === "narration" ? "assistant" : m.role, content: m.characterName ? `【${m.characterName}】\n${m.rawContent}` : m.rawContent });
  }
  return messages;
}
...
export async function generateXiaShuCompletion(characterIds: string[], worldId: string, history: XiaShuMessage[], signal?: AbortSignal) {
  ...
  const binding = resolveBinding(bindings, first, "xiashu");
  ...
  const raw = await sendLLMRequest(
    api,
    preset,
    buildXiaShuPrompt(characterIds, worldId, history),
    resources.regexes,
    { characterName: getXiaShuCharacters(characterIds).map(c => c.name).join("、"), userName: "用户" },
